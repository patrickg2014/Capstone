...
public class CameraActivity extends Activity implements SensorEventListener {
	...
	public ArrayList<Marker> currentlyInside = new ArrayList();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		super.onCreate(savedInstanceState); // pulls and saved state for the
											// activity
		...
		mSensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);// sets up a sensor manager to help with gps and compass data
	...
	public static Camera getCameraInstance() // starts up a camera if the device
												// has one available
	{
		mainCam = null;
		try {
	...
	public void whatshouldwesee() {
		...
		for (Marker m : marks) // loops through all a markers to see which ones
								// are within a certain radius of us
		{
		...
		for (Marker m : currentlyNear)// a loop to see which of those currently near markers are within our angle of view
			...
			if (Math.abs(myloc.bearingTo(location) - heading) < 50||(Math.abs(myloc.bearingTo(location) - heading) > 300)) // the check to see which ones are
			{
				currentlyvisable.add(m);
				Log.d("near", m.getTitle());
				theText=theText+" "+m.getTitle(); // update the text being written
				camover.xPos=((myloc.bearingTo(location) - heading)+50)*13;
		...
		for(Marker m : currentlyvisable){
			...
			location.setLatitude(m.getPosition().latitude);
			location.setLongitude(m.getPosition().longitude);
			if (myloc.bearingTo(location) - heading < 0){
				
				
			}
		}
		}
		camover.setDisplayText(theText);
	}
	...
	public void onMyLocationChange(Location location) {
			...
			geo = geoField.getDeclination(); // add the adjustment value for
												// true north magnetic north
												// difference
			currentLocation = getGPS(); // update our location
			whatshouldwesee(); // update the markers that we are near and should
			Log.d("GPS", currentLocation.getLatitude()+ "  "+currentLocation.getLatitude());
								// be able to view
	...
	/*
	 * This is a listener for when the compass sensor has changed 
	 * (non-Javadoc)
	 * 
	 * @see
	...
	public void onSensorChanged(SensorEvent event) {
		/*
		 * Rounds the current heading to full degrees while accounting for
		 * movement around the x axis and adding 90 to account for portrait
		 * orientation
		 */
		heading = ((Math.round(event.values[0] + event.values[1]) + 90) + geo) % 360; 
		...
		{
			whatshouldwesee();// update the information about what we are near
								// and what is in our view
		}
	}
